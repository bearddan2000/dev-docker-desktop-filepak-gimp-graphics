...
function start-up(){
  ...
  sudo docker build -t flatpak .

  echo "$info_base running image" >> $logfile

  sudo docker run --name flatpak -ti -d -e DISPLAY=:0 \
    --privileged -v /lib/modules:/lib/modules:ro \
    -v /tmp/.X11-unix:/tmp/.X11-unix \
    flatpak

  sudo docker restart flatpak

  # Security patch
  # sudo docker exec -it flatpak mount -t securityfs securityfs /sys/kernel/security

  # As root
  sudo docker exec -it flatpak flatpak install -y flathub $app
  # sudo docker exec -it flatpak flatpak info $app
  sudo docker exec -it -u developer flatpak flatpak run $app
